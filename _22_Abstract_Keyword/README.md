# Chapter 22: The `abstract` Keyword (Abstract Classes & Methods)
...
| File | Demonstrates |
|------|--------------|
| `_22_Abstract_Keyword_Code.java` | An abstract class `Car` with one abstract method (`drive`) and one concrete method (`playMusic`), implemented by the concrete class `WagonR` |
...
public class _22_Abstract_Keyword_Code {
...
All classes declare `package _22_Abstract_Keyword;`. Compile and run from the
parent directory using the package path:

```bash
# Compile
javac _22_Abstract_Keyword/_22_Abstract_Keyword_Code.java

# Run
java _22_Abstract_Keyword._22_Abstract_Keyword_Code
```
