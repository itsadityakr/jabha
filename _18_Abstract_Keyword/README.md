# Chapter 18: The `abstract` Keyword (Abstract Classes & Methods)
...
| File | Demonstrates |
|------|--------------|
| `_18_Abstract_Keyword_Code.java` | An abstract class `Car` with one abstract method (`drive`) and one concrete method (`playMusic`), implemented by the concrete class `WagonR` |
...
public class _18_Abstract_Keyword_Code {
...
All classes declare `package _18_Abstract_Keyword;`. Compile and run from the
parent directory using the package path:

```bash
# Compile
javac _18_Abstract_Keyword/_18_Abstract_Keyword_Code.java

# Run
java _18_Abstract_Keyword._18_Abstract_Keyword_Code
```
