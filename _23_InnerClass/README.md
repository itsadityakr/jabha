# Chapter 23: Inner Classes (Member, Static Nested & Anonymous)
...
| File | Demonstrates |
|------|--------------|
| `_23_1_InnerClass_Code.java` | A **member (non-static) inner class** `B` and a **static nested class** `C` inside an outer class `A` |
| `_23_2_AnonymousInnerClass_Code.java` | An **anonymous inner class** -- a one-off, unnamed subclass of `A` -- contrasted with a normal named subclass `B` |
| `_23_3_AbstractAnonymousInnerClass_Code.java` | An **anonymous inner class implementing an abstract class** `A` |

> **Important:** all three files declare a top-level `class A` in the **same
> package** (`_23_InnerClass`). That means they **cannot be compiled together**
> (`javac _23_InnerClass/*.java` fails with *"duplicate class: A"*). Compile and
...
public class _23_1_InnerClass_Code {
...
### `_23_2_AnonymousInnerClass_Code.java`
...
public class _23_2_AnonymousInnerClass_Code {
    public static void main(String[] args) {
...
### `_23_3_AbstractAnonymousInnerClass_Code.java`

```java
abstract class A { abstract public void show(); }

public class _23_3_AbstractAnonymousInnerClass_Code {
    public static void main(String[] args) {
...
Each file declares `package _23_InnerClass;`. Because **all three files define a
top-level `class A`**, they conflict if compiled together -- compile and run them
**one at a time**:

```bash
# Member & static nested inner classes
javac _23_InnerClass/_23_1_InnerClass_Code.java
java  _23_InnerClass._23_1_InnerClass_Code

# Anonymous inner class
javac _23_InnerClass/_23_2_AnonymousInnerClass_Code.java
java  _23_InnerClass._23_2_AnonymousInnerClass_Code

# Anonymous subclass of an abstract class
javac _23_InnerClass/_23_3_AbstractAnonymousInnerClass_Code.java
java  _23_InnerClass._23_3_AbstractAnonymousInnerClass_Code
```

> Do **not** use `javac _23_InnerClass/*.java` here -- it fails with
> *"duplicate class: \_16\_InnerClass.A"* because three files each declare `class A`.

### Expected Output

`_23_1_InnerClass_Code`:
...
`_23_2_AnonymousInnerClass_Code`:
...
`_23_3_AbstractAnonymousInnerClass_Code`:
...
```bash
javac _23_InnerClass/*.java   # fails: duplicate class A
```
...
(Chapter 15), Polymorphism (Chapter 17), and the `abstract` keyword (Chapter 22),
