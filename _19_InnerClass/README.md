# Chapter 19: Inner Classes (Member, Static Nested & Anonymous)
...
| File | Demonstrates |
|------|--------------|
| `_19_1_InnerClass_Code.java` | A **member (non-static) inner class** `B` and a **static nested class** `C` inside an outer class `A` |
| `_19_2_AnonymousInnerClass_Code.java` | An **anonymous inner class** -- a one-off, unnamed subclass of `A` -- contrasted with a normal named subclass `B` |
| `_19_3_AbstractAnonymousInnerClass_Code.java` | An **anonymous inner class implementing an abstract class** `A` |

> **Important:** all three files declare a top-level `class A` in the **same
> package** (`_19_InnerClass`). That means they **cannot be compiled together**
> (`javac _19_InnerClass/*.java` fails with *"duplicate class: A"*). Compile and
...
public class _19_1_InnerClass_Code {
...
### `_19_2_AnonymousInnerClass_Code.java`
...
public class _19_2_AnonymousInnerClass_Code {
    public static void main(String[] args) {
...
### `_19_3_AbstractAnonymousInnerClass_Code.java`

```java
abstract class A { abstract public void show(); }

public class _19_3_AbstractAnonymousInnerClass_Code {
    public static void main(String[] args) {
...
Each file declares `package _19_InnerClass;`. Because **all three files define a
top-level `class A`**, they conflict if compiled together -- compile and run them
**one at a time**:

```bash
# Member & static nested inner classes
javac _19_InnerClass/_19_1_InnerClass_Code.java
java  _19_InnerClass._19_1_InnerClass_Code

# Anonymous inner class
javac _19_InnerClass/_19_2_AnonymousInnerClass_Code.java
java  _19_InnerClass._19_2_AnonymousInnerClass_Code

# Anonymous subclass of an abstract class
javac _19_InnerClass/_19_3_AbstractAnonymousInnerClass_Code.java
java  _19_InnerClass._19_3_AbstractAnonymousInnerClass_Code
```

> Do **not** use `javac _19_InnerClass/*.java` here -- it fails with
> *"duplicate class: \_16\_InnerClass.A"* because three files each declare `class A`.

### Expected Output

`_19_1_InnerClass_Code`:
...
`_19_2_AnonymousInnerClass_Code`:
...
`_19_3_AbstractAnonymousInnerClass_Code`:
...
```bash
javac _19_InnerClass/*.java   # fails: duplicate class A
```
...
(Chapter 11), Polymorphism (Chapter 13), and the `abstract` keyword (Chapter 18),
